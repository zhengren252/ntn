...
async def request_middleware(request: Request, call_next):
    ...
    # 记录请求
    logger.info(f"收到请求: {request.method} {request.url} rid={request_id}")
    ...
    # 记录响应
    logger.info(f"响应完成: {response.status_code}, 耗时: {process_time:.4f}秒 rid={request_id}")
...
async def global_exception_handler(request: Request, exc: Exception):
    ...
    rid = getattr(request.state, "request_id", None) or request.headers.get("X-Request-ID") or "unknown"
